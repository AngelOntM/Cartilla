...
export const createVacuna = async (req, res) => {
    try {
        const connection = await connect()
        const [rows] = await connection.query("INSERT INTO vacuna(RAZ_NUMCTRL,VAC_NOMBRE,VAC_DESC) VALUES (?, ?, ?)",
            [
...
export const deleteVacuna = async (req, res) => {
    try {
        const connection = await connect()
        await connection.query('DELETE FROM vacuna WHERE VAC_NUMCTRL = ?',
            [
                req.params.id
            ])
        res.sendStatus(204)
