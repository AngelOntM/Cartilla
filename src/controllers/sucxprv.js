...
export const createSucxprv = async (req, res) => {
    try {
        const connection = await connect()
        const [rows] = await connection.query("INSERT INTO sucxprv(PRV_NUMCTRL, SXP_NOMBRE, SXP_DIRECCION, SXP_COLONIA, SXP_CP, SXP_ENCARGADO, SXP_TELEFONO, SXP_LAT, SXP_LON, SXP_CORREO, SXP_CONTRA) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
            [
                req.body.PRV_NUMCTRL,
                req.body.SXP_NOMBRE,
                req.body.SXP_DIRECCION,
                req.body.SXP_COLONIA,
                req.body.SXP_CP,
                req.body.SXP_ENCARGADO,
                req.body.SXP_TELEFONO,
                req.body.SXP_LAT,
                req.body.SXP_LON,
                req.body.SXP_CORREO,
                req.body.SXP_CONTRA
...
export const deleteSucxprv = async (req, res) => {
    try {
        const connection = await connect()
        const [rows] = await connection.query('DELETE FROM sucxprv WHERE SXP_NUMCTRL = ?',
            [
                req.params.id
            ])
        res.sendStatus(204)
...
export const updateSucxprv = async (req, res) => {
    try {
        const connection = await connect()
        const [rows] = await connection.query('UPDATE sucxprv SET ? WHERE SXP_NUMCTRL = ?',
            [
                req.body,
                req.params.id
            ])
