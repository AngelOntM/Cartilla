...
export const createSerxsuc = async (req, res) => {
    try {
        const connection = await connect()
        const [rows] = await connection.query("INSERT INTO Serxsuc(SXP_NUMCTRL, SXP_NOMBRE, SXS_NOMBRE, SXS_DESCRIPTION, SXS_PRECIO) VALUES (?, ?, ?, ?, ?)",
            [
                req.body.SXP_NUMCTRL,
                req.body.SXP_NOMBRE,
                req.body.SXS_NOMBRE,
                req.body.SXS_DESCRIPTION,
                req.body.SXS_PRECIO
            ])
        res.json({
            id: rows.insertId,
...
export const deleteSerxsuc = async (req, res) => {
    try {
        const connection = await connect()
        const [rows] = await connection.query('DELETE FROM Serxsuc WHERE SXS_NUMCTRL = ?',
            [
                req.params.id
            ])
        res.sendStatus(204)
...
export const updateSerxsuc = async (req, res) => {
    try {
        const connection = await connect()
        const [rows] = await connection.query('UPDATE Serxsuc SET ? WHERE SXS_NUMCTRL = ?',
            [
                req.body,
                req.params.id
            ])
