...
export const createSerxmas = async (req, res) => {
    try {
        const connection = await connect()
        const [rows] = await connection.query("INSERT INTO Serxmas(MAS_NUMCTRL, SXM_FECHA, SXM_PRECIO) VALUES (?, ?, ?)",
            [
...
export const deleteSerxmas = async (req, res) => {
    try {
        const connection = await connect()
        const [rows] = await connection.query('DELETE FROM Serxmas WHERE SXM_NUMCTRL = ?',
            [
                req.params.id
            ])
        res.sendStatus(204)
...
export const updateSerxmas = async (req, res) => {
    try {
        const connection = await connect()
        const [rows] = await connection.query('UPDATE Serxmas SET ? WHERE SXM_NUMCTRL = ?',
            [
                req.body,
                req.params.id
            ])
