...
export const createVacxmas = async (req, res) => {
    try {
        const connection = await connect()
        const [rows] = await connection.query("INSERT INTO Vacxmas(MAS_NUMCTRL, PRV_NUMCTRL, VAC_NUMCTRL, VXM_PESO, VXM_FECHA, VXM_FECHAAPLICA) VALUES (?, ?, ?, ?, ?, ?)",
            [
                req.body.MAS_NUMCTRL,
                req.body.PRV_NUMCTRL,
                req.body.VAC_NUMCTRL,
                req.body.VXM_PESO,
                req.body.VXM_FECHA,
                req.body.VXM_FECHAAPLICA
            ])
        res.json({
            id: rows.insertId,
            ...req.body
...
export const deleteVacxmas = async (req, res) => {
    try {
        const connection = await connect()
        const [rows] = await connection.query('DELETE FROM Vacxmas WHERE VXM_NUMCTRL = ?',
            [
                req.params.id
            ])
        res.sendStatus(204)
...
export const updateVacxmas = async (req, res) => {
    try {
        const connection = await connect()
        const [rows] = await connection.query('UPDATE Vacxmas SET ? WHERE VXM_NUMCTRL = ?',
            [
                req.body,
                req.params.id
            ])
