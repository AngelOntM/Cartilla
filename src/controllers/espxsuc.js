...
export const createEspxsuc = async (req, res) => {
    try {
        const connection = await connect()
        const [rows] = await connection.query("INSERT INTO Espxsuc(ESP_NUMCTRL, SXP_NUMCTRL) VALUES (?, ?)",
            [
                req.body.ESP_NUMCTRL,
...
export const deleteEspxsuc = async (req, res) => {
    try {
        const connection = await connect()
        const [rows] = await connection.query('DELETE FROM Espxsuc WHERE ESP_NUMCTRL = ?',
            [
                req.params.id
            ])
        res.sendStatus(204)
...
export const updateEspxsuc = async (req, res) => {
    try {
        const connection = await connect()
        const [rows] = await connection.query('UPDATE Espxsuc SET ? WHERE ESP_NUMCTRL = ?',
            [
                req.body,
                req.params.id
            ])
