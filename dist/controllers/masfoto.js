...
export const createMasfoto = async (req, res) => {
  try {
    const connection = await connect();
    const [rows] = await connection.query("INSERT INTO Masfoto(MAS_NUMCTRL, MAF_FOTO) VALUES (?, ?)", [req.body.MAS_NUMCTRL, req.body.MAF_FOTO]);
    res.json({
      id: rows.insertId,
      ...req.body
    });
...
export const deleteMasfoto = async (req, res) => {
  try {
    const connection = await connect();
    const [rows] = await connection.query('DELETE FROM Masfoto WHERE MAF_NUMCTRL = ?', [req.params.id]);
    res.sendStatus(204);
  } catch (error) {
    res.sendStatus(400);
  }
...
export const updateMasfoto = async (req, res) => {
  try {
    const connection = await connect();
    const [rows] = await connection.query('UPDATE Masfoto SET ? WHERE MAF_NUMCTRL = ?', [req.body, req.params.id]);
    res.json(rows);
  } catch (error) {
    res.sendStatus(400);
  }
