...
export const createRaza = async (req, res) => {
  try {
    const connection = await connect();
    const [rows] = await connection.query("INSERT INTO raza(RAZ_NOMBRE) VALUES (?)", [req.body.RAZ_NOMBRE]);
    res.json({
      id: rows.insertId,
      ...req.body
    });
...
export const deleteRaza = async (req, res) => {
  try {
    const connection = await connect();
    const [rows] = await connection.query('DELETE FROM raza WHERE RAZ_NUMCTRL = ?', [req.params.id]);
    res.sendStatus(204);
  } catch (error) {
    res.sendStatus(400);
  }
...
export const updateRaza = async (req, res) => {
  try {
    const connection = await connect();
    const [rows] = await connection.query('UPDATE raza SET ? WHERE RAZ_NUMCTRL = ?', [req.body, req.params.id]);
    res.json(rows);
  } catch (error) {
    res.sendStatus(400);
  }
