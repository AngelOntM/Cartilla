...
export const createProxmen = async (req, res) => {
  try {
    const connection = await connect();
    const [orden] = await connection.query('SELECT * FROM proxmen WHERE MEN_NUMCTRL = ? AND PXM_ORDEN = ?', [req.body.MEN_NUMCTRL, req.body.PXM_ORDEN]);

    if (orden[0] != undefined) {
      return res.sendStatus(400);
    }

    const [rows] = await connection.query("INSERT INTO proxmen(PRG_NUMCTRL, MEN_NUMCTRL, proxmen.PXM_ORDEN) VALUES (?, ?, ?)", [req.body.PRG_NUMCTRL, req.body.MEN_NUMCTRL, req.body.PXM_ORDEN]);
    res.json({
      id: rows.insertId,
      ...req.body
    });
...
export const deleteProxmen = async (req, res) => {
  try {
    ...
    await connection.query('DELETE FROM proxmen WHERE PXM_NUMCTRL = ?', [req.params.id]);
    const [up] = await connection.query('UPDATE proxmen SET PXM_ORDEN = PXM_ORDEN - 1 WHERE PXM_ORDEN > ? AND MEN_NUMCTRL = ?', [orden[0].PXM_ORDEN, orden[0].MEN_NUMCTRL]);
    res.sendStatus(204);
  } catch (error) {
    res.sendStatus(400);
  }
  ...
  try {
    ...
    if (req.body.PXM_ORDEN) {
      ...
      if (orden[0].PXM_ORDEN < req.body.PXM_ORDEN) {
        await connection.query('UPDATE proxmen SET PXM_ORDEN = PXM_ORDEN - 1 WHERE MEN_NUMCTRL = ? AND PXM_ORDEN > ? AND PXM_ORDEN <= ?', [orden[0].MEN_NUMCTRL, orden[0].PXM_ORDEN, req.body.PXM_ORDEN]);
      } else if (orden[0].PXM_ORDEN > req.body.PXM_ORDEN) {
        await connection.query('UPDATE proxmen SET PXM_ORDEN = PXM_ORDEN + 1 WHERE MEN_NUMCTRL = ? AND PXM_ORDEN < ? AND PXM_ORDEN >= ?', [orden[0].MEN_NUMCTRL, orden[0].PXM_ORDEN, req.body.PXM_ORDEN]);
      }
    }

    const [rows] = await connection.query('UPDATE proxmen SET ? WHERE PXM_NUMCTRL = ?', [req.body, req.params.id]);
    res.json(rows);
  } catch (error) {
    res.sendStatus(400);
  }
