...
export const createProxusu = async (req, res) => {
  try {
    const connection = await connect();
    const [rows] = await connection.query("INSERT INTO proxusu(TIU_NUMCTRL, PRG_NUMCTRL) VALUES (?, ?)", [req.body.TIU_NUMCTRL, req.body.PRG_NUMCTRL]);
    res.json({
      id: rows.insertId,
      ...req.body
    });
...
export const deleteProxusu = async (req, res) => {
  try {
    const connection = await connect();
    const [rows] = await connection.query('DELETE FROM proxusu WHERE PXU_NUMCTRL = ?', [req.params.id]);
    res.sendStatus(204);
  } catch (error) {
    res.sendStatus(400);
  }
...
export const updateProxusu = async (req, res) => {
  try {
    const connection = await connect();
    const [rows] = await connection.query('UPDATE proxusu SET ? WHERE PXU_NUMCTRL = ?', [req.body, req.params.id]);
    res.json(rows);
  } catch (error) {
    res.sendStatus(400);
  }
