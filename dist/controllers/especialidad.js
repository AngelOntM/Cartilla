...
export const createEspecialidad = async (req, res) => {
  try {
    const connection = await connect();
    const [rows] = await connection.query("INSERT INTO especialidad(ESP_NOMBRE,ESP_DESC) VALUES (?,?)", [req.body.ESP_NOMBRE, req.body.ESP_DESC]);
    res.json({
      id: rows.insertId,
      ...req.body
    });
...
export const deleteEspecialidad = async (req, res) => {
  try {
    const connection = await connect();
    const [rows] = await connection.query('DELETE FROM especialidad WHERE ESP_NUMCTRL = ?', [req.params.id]);
    res.sendStatus(204);
  } catch (error) {
    res.sendStatus(400);
  }
...
export const updateEspecialidad = async (req, res) => {
  try {
    const connection = await connect();
    const [rows] = await connection.query('UPDATE especialidad SET ? WHERE ESP_NUMCTRL = ?', [req.body, req.params.id]);
    res.json(rows);
  } catch (error) {
    res.sendStatus(400);
  }
