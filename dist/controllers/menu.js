...
export const createMenu = async (req, res) => {
  try {
    const connection = await connect();
    const [orden] = await connection.query('SELECT * FROM menu WHERE MEN_ORDEN = ? AND SUM_NUMCTRL = ?', [req.body.MEN_ORDEN, req.body.SUM_NUMCTRL]);

    if (orden[0] != undefined) {
      return res.sendStatus(400);
    }

    const [rows] = await connection.query("INSERT INTO menu(MEN_CLAVE,MEN_NOMBRE,MEN_ICON,MEN_DESC,MEN_ORDEN,SUM_NUMCTRL) VALUES (?, ?, ?, ?, ?, ?)", [req.body.MEN_CLAVE, req.body.MEN_NOMBRE, req.body.MEN_ICON, req.body.MEN_DESC, req.body.MEN_ORDEN, req.body.SUM_NUMCTRL]);
    res.json({
      id: rows.insertId,
      ...req.body
    });
...
export const deleteMenu = async (req, res) => {
  try {
    ...
    await connection.query('DELETE FROM menu WHERE MEN_NUMCTRL = ?', [req.params.id]);
    const [up] = await connection.query('UPDATE menu SET MEN_ORDEN = MEN_ORDEN - 1 WHERE MEN_ORDEN > ? AND SUM_NUMCTRL = ?', [orden[0].MEN_ORDEN, orden[0].SUM_NUMCTRL]);
    res.sendStatus(204);
  } catch (error) {
    res.sendStatus(400);
  }
  ...
  try {
    ...
    if (req.body.MEN_ORDEN) {
      ...
      if (orden[0].MEN_ORDEN < req.body.MEN_ORDEN) {
        await connection.query('UPDATE menu SET MEN_ORDEN = MEN_ORDEN - 1 WHERE SUM_NUMCTRL = ? AND MEN_ORDEN > ? AND MEN_ORDEN <= ?', [orden[0].SUM_NUMCTRL, orden[0].MEN_ORDEN, req.body.MEN_ORDEN]);
      } else if (orden[0].MEN_ORDEN > req.body.MEN_ORDEN) {
        await connection.query('UPDATE menu SET MEN_ORDEN = MEN_ORDEN + 1 WHERE SUM_NUMCTRL = ? AND MEN_ORDEN < ? AND MEN_ORDEN >= ?', [orden[0].SUM_NUMCTRL, orden[0].MEN_ORDEN, req.body.MEN_ORDEN]);
      }
    }

    const [rows] = await connection.query('UPDATE menu SET ? WHERE MEN_NUMCTRL = ?', [req.body, req.params.id]);
    res.json(rows);
  } catch (error) {
    res.sendStatus(400);
  }
