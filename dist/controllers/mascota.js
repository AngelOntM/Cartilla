...
export const createMascota = async (req, res) => {
  try {
    const connection = await connect();
    const [rows] = await connection.query("INSERT INTO mascota(PRO_NUMCTRL, MAS_NOMBRE, MAS_FECHANAC, MAS_RAZA, RAZ_NUMCTRL, MAS_SEXO, MAS_SENPAR) VALUES (?, ?, ?, ?, ?, ?, ?)", [req.body.PRO_NUMCTRL, req.body.MAS_NOMBRE, req.body.MAS_FECHANAC, req.body.MAS_RAZA, req.body.RAZ_NUMCTRL, req.body.MAS_SEXO, req.body.MAS_SENPAR]);
    res.json({
      id: rows.insertId,
      ...req.body
    });
...
export const deleteMascota = async (req, res) => {
  try {
    const connection = await connect();
    const [rows] = await connection.query('DELETE FROM mascota WHERE MAS_NUMCTRL = ?', [req.params.id]);
    res.sendStatus(204);
  } catch (error) {
    res.sendStatus(400);
  }
...
export const updateMascota = async (req, res) => {
  try {
    const connection = await connect();
    const [rows] = await connection.query('UPDATE mascota SET ? WHERE MAS_NUMCTRL = ?', [req.body, req.params.id]);
    res.json(rows);
  } catch (error) {
